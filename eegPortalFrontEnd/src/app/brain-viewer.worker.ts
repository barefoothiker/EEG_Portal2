...
addEventListener('message', ({ data }) => {
  ...
  // / <reference lib="webworker" />
  // @Output() webWorkerCompletion = new EventEmitter<any[]>();
  ...
  // self.addEventListener('message', ({ data }) => {
    ...
    // if (data === 'do some work') {
      ...
      for(var i = 0; i < (data["imagePrefixes"]).length; i++){
         ...
         // console.log( " BEFORE fullScreenBrainDataArray len = " + this.fullScreenBrainData[i]);

         // input

         fullScreenBrainDataArray = Base64Binary.decodeArrayBuffer(data.fullScreenBrainData[i]);
         // console.log( " fullScreenBrainDataArray len = " + fullScreenBrainDataArray.byteLength);
      ...
      // this.webWorkerCompletion.emit(outdata);
